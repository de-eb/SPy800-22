...
from tests.utils import Multiple
...
if __name__ == "__main__":
    ...
    # test = FrequencyTest(seq_len=1000, seq_num=3, proc_num=1)
    # test.read_bits(".debug/testsequence.bin", fmt=test.ReadAs.BIGENDIAN)
    # test.run()
    # freq = BlockFrequencyTest(seq_len=1000, seq_num=10, proc_num=2)
    # freq.read_bits(".debug/testsequence.bin", fmt=freq.ReadAs.BIGENDIAN)
    # freq.run()

    test = Multiple(seq_len=1000, seq_num=10, proc_num=2)
    test.read_bits(".debug/testsequence.bin", fmt=test.ReadAs.BIGENDIAN)
    test.run()
    test.save_report(".debug/report.csv")
    print()
    ...
    # sts = utils.STS(f_path=None, f_fmt=None, t_sel=[], seq_size=None, seq_num=None)
